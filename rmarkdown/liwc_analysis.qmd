...
This script uses the scores of LIWC features and merges with country-year level meta data. `"data/raw/controls/controls.csv"` has a battery of country-year level variables that might potentially confound the statistical modeling. With [`liwc_meta`](https://drive.google.com/file/d/1HBJpHd53BI0uq4z9rTkMsj2uEpKaq6aj/view?usp=share_link) dataset, at the country-year level, I run a series of statistical models that probe the relationship between linguistic features and sentiment scores of that speech. To preview, LIWC features alone have a strong predictive power on regime types, even without the help of meta data. 
...
#removing variables with too many missing values (missing above 70%)
liwc_df <- liwc_df %>%
  dplyr::select(
...
liwc_df <-  liwc_master[liwc_master$year < 2021, ]
validation <- liwc_master[liwc_master$year >= 2021, ]

# subset liwc features only
liwc_inputs<-liwc_df[, 5:122]
id <- liwc_df[, 1:3]
y <- liwc_df[, 149]

controls <- liwc_df[, c(126:148, 150:191)] #subsetting 65 meta variables
...
Confusion matrix:
     0    1 class.error
0 2563  859   0.2510228
1  784 2561   0.2343797
...
## Trial 2: excluding WC, WPS, and Period
