...
sov_tokens <- sov_tokens[!is.na(docvars(sov_tokens)$democracy), ]
...
sov_wv_regime <- dem_group(sov_dem,
                           groups = sov_dem@docvars$democracy)
...
get_cos_sim(x = sov_tokens,
            ...
            features = c("intervene", "interfere", "right", "security", "border", "independence"),
            pre_trained = glove,
            transform = TRUE,
            transform_matrix = cr_transform,
            bootstrap = TRUE,
...
```{r}
set.seed(2024)
model1 <- conText(formula =  . ~ democracy,
