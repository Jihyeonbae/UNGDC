...
-   [x] Experiment2: 10-year, 5 overlap, saved in `moving_0218_2`. tfidf option: none. 20 topics
-   [ ] Experiment3: 10-year, 5 overlap, saved in `moving_0219`. tfidf option: count and inverse. 20 topics
...
```{r}
lda_generator <- function(corpus, span_levels, window_size = 10, overlap = 5, num_topics = 20) {
...
output_directory <- file.path("../output/lda/moving_0218_2")
...
#| warning: FALSE
#| echo : FALSE
generate_heatmap <- function(model1, model2, span) {
  phi1 <- model1$phi
  phi2 <- model2$phi
  ...
  heatmap_title <- paste("Correlation - Time Window:", span)
...
for (i in 1:length(lda_models)) {
  if (i < length(lda_models)) {
    ...
    span <- span_levels[i]
    result <- generate_heatmap(model1, model2, span)
