...
This version *does* use tf-idf for LDA analysis. 
It also includes updated options from the `quanteda` R package. 
...
-   Current parameters in the below code sets the number of topics as 5, and uses the TF-IDF scores.
...
lda_generator <- function(corpus, span_levels, num_topics = 5) {
...
output_directory <- "../output/lda/decade_0120_replicate"
